...
public class Tetris3D : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // MOVE TETROMINO //
        if (Input.GetKeyDown(KeyCode.LeftArrow) || Input.GetKeyDown(KeyCode.A) || state.IsTriggered(WiiU.GamePadButton.Left))
        {
            MoveTetromino(Vector3.left);
        }
        else if (Input.GetKeyDown(KeyCode.RightArrow) || Input.GetKeyDown(KeyCode.D) || state.IsTriggered(WiiU.GamePadButton.Right))
        {
            MoveTetromino(Vector3.right);
        }
        else if (Input.GetKeyDown(KeyCode.UpArrow) || Input.GetKeyDown(KeyCode.W) || state.IsTriggered(WiiU.GamePadButton.Up))
        {
            MoveTetromino(Vector3.forward);
        }
        else if (Input.GetKeyDown(KeyCode.DownArrow) || Input.GetKeyDown(KeyCode.S) || state.IsTriggered(WiiU.GamePadButton.Down))
        {
            MoveTetromino(Vector3.back);
        }
        else if (Input.GetKeyDown(KeyCode.Q) || state.IsTriggered(WiiU.GamePadButton.R))
        {
            RotateTetromino(Vector3.up, 90); // Rotate around Y-axis for 3D rotation
        }
        else if (Input.GetKeyDown(KeyCode.E) || state.IsTriggered(WiiU.GamePadButton.Down) || state.IsTriggered(WiiU.GamePadButton.L))
        {
        ...
        // MOVE DOWN //
        if (Input.GetKey(KeyCode.Space) || state.IsPressed(WiiU.GamePadButton.X))
        {
        ...
        // SWAP BLOCK //
        if (Input.GetKeyDown(KeyCode.Y) || state.IsPressed(WiiU.GamePadButton.Y))
        {
            HoldTetromino();
        }
